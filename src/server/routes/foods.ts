...
app.post('/', async (c) => {
  const db = c.env.DB
  const userId = c.get('userId')
  const { name, calories_per_100g } = await c.req.json()
  ...
  try {
    const { meta } = await db.prepare(
      'INSERT INTO foods (user_id, name, calories_per_100g) VALUES (?, ?, ?)'
    ).bind(userId, name, calories_per_100g).run()
...
app.put('/:id', async (c) => {
  ...
  await db.prepare(
    'UPDATE foods SET name = ?, calories_per_100g = ? WHERE id = ?'
  ).bind(name, calories_per_100g, id).run()
